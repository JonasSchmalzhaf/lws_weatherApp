...
class WeatherService(private val weatherRepository: WeatherRepository, private val restTemplateBuilder: RestTemplateBuilder) {
    fun getWeatherForecast(city: String, days: Int): List<WeatherData> {
        val safeCity = city.lowercase().replaceFirstChar { if (it.isLowerCase()) it.titlecase(Locale.getDefault()) else it.toString() }

        if (weatherRepository.findByCity(safeCity).isEmpty()) {
            getForecastFromOpenWeatherMap(safeCity)
        }

        if (weatherRepository.findByCity(safeCity).filter { LocalDate.now() <= it.forecastDate && it.forecastDate <= LocalDate.now().plusDays(days.toLong()) }.size < days) {
            refreshWeatherForecastData()
        }

        return weatherRepository.findByCity(safeCity).filter { LocalDate.now() <= it.forecastDate && it.forecastDate <= LocalDate.now().plusDays(days.toLong()) }
    }
    ...
    fun forecastToDaily(forecast: WeatherForecastModel.WeatherForecast): List<WeatherData> {
        ...
        groupedList.forEach { (date, dayForecast) -> dailyForecast.add(WeatherData(
            ...
            description = if (dayForecast.filter { it.weather.first().icon.contains("d") }.isEmpty())  dayForecast.groupingBy { it.weather.first().description }.eachCount().maxBy { it.value }.key else dayForecast.filter { it.weather.first().icon.contains("d") }.groupingBy { it.weather.first().description }.eachCount().maxBy { it.value }.key,
            iconCode = if(dayForecast.filter { it.weather.first().icon.contains("d") }.isEmpty()) dayForecast.groupingBy { it.weather.first().icon }.eachCount().maxBy { it.value }.key else dayForecast.filter { it.weather.first().icon.contains("d") }.groupingBy { it.weather.first().icon }.eachCount().maxBy { it.value }.key,
        ))}
